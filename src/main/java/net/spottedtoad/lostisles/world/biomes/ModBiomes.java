...
public class ModBiomes {
    private static Registry<? super String> Biome;
    ...
    public static void bootstrap(BootstrapContext<Biome> context) {
        context.register(PRIMORDIAL_ISLAND, createIslandBiome(context));
        context.register(PRIMORDIAL_OCEAN, createOceanBiome(context, false));
        context.register(DEEP_PRIMORDIAL_OCEAN, createOceanBiome(context, true));
    }

    private static Biome createIslandBiome(BootstrapContext<Biome> context) {
        MobSpawnSettings.Builder spawns = new MobSpawnSettings.Builder();
        BiomeDefaultFeatures.farmAnimals(spawns);
        BiomeDefaultFeatures.monsters(spawns, 1, 1, 1, 1, false);
    ...
    private static Biome createOceanBiome(BootstrapContext<Biome> context, boolean isDeep) {
        MobSpawnSettings.Builder spawns = new MobSpawnSettings.Builder();
        BiomeGenerationSettings.Builder geo = new BiomeGenerationSettings.Builder(
                context.lookup(Registries.PLACED_FEATURE),
                context.lookup(Registries.CONFIGURED_CARVER)
